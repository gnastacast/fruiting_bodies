...
p.loadURDF(os.path.join(dataDir, "plane100.urdf"), useMaximalCoordinates=True)
...
visualShapeId = p.createVisualShape(shapeType=p.GEOM_MESH,fileName=os.path.join(dataDir, "duck.obj"), rgbaColor=[1,1,1,1], specularColor=[0.4,.4,0], visualFramePosition=shift, meshScale=meshScale)
collisionShapeId = p.createCollisionShape(shapeType=p.GEOM_MESH, fileName=os.path.join(dataDir, "duck_vhacd.obj"), collisionFramePosition=shift,meshScale=meshScale)
...
objects = [os.path.join(dataDir, "duck.obj"), os.path.join(dataDir, "duck_vhacd.obj")]
currentColor = 0
		...
		if ((e[0] == 2) and (e[3]==0) and (e[4]& p.KEY_WAS_TRIGGERED)):
			...
			for l in range(len(rayInfo)):
				...
				if (objectUid>=1):
					...
					p.changeVisualShape(objectUid,-1,fileName=objects[currentColor % 2])
					currentColor+=1
