...
p.connect(p.GUI)
planeId = p.loadURDF(fileName=os.path.join(dataDir, "plane.urdf"),baseOrientation=[0.25882,0,0,0.96593])
p.loadURDF(fileName=os.path.join(dataDir, "cube.urdf"),basePosition=[0,0,2])
cubeId = p.loadURDF(fileName=os.path.join(dataDir, "cube.urdf"),baseOrientation=[0,0,0,1],basePosition=[0,0,4])
